import {app, Tray, Menu, nativeImage, Notification, shell, dialog} from 'electron';
...
export class ElectronApp {
        ...
        return new Promise((resolve) => {
            this.app.on('ready', () => {
                ...
                new Notification({
                    ...
                    body: 'Получить доступ можно через значок в трее'
                }).show();
                resolve(null);
            });
        });
    ...
    static createTray() {
        ...
        this.tray.on('click', () => this.openBrowser());


        this.tray.setToolTip('STAS Control Panel');
        this.tray.setContextMenu(contextMenu);
    }

    static showErrorDialogSync(message: string, title?: string) {
    ...
    static async openBrowser() {
        if (this.isRunning) await shell.openExternal('http://localhost:' + (process.env.PORT || 3000))
    }
}
